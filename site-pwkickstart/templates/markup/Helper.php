...
class Helper
{
  public static function renderModuleTemplate($templateString, $dataFromModuleCall = [], $alternativeFile = null) {
    ...
    $templateFile = "$templateString.php";
    if($alternativeFile) $templateFile = "$alternativeFile.php";

    $t = new TemplateFile(wire('config')->paths->templates . "dist/modules/$templateString/$templateFile");
  ...
  public static function renderModifiers($data, $prefix = null) {
    if(!isset($data['modifiers'])) return;

    $modifierString = '';

    foreach($data['modifiers'] as $modifier) {
      $modifierString .= $prefix . $modifier . " ";
    }
